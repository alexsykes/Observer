...
$sql = "SELECT t.id, eventname, date, club, v.name as location FROM up93k_entryman_trial AS t LEFT JOIN up93k_entryman_venue AS v ON t.venue_id = v.id WHERE t.published = 1 AND `date` > DATE(NOW()) ORDER BY `date` ASC";
...
$stmt->bind_result($id, $eventname, $date, $club, $location);
